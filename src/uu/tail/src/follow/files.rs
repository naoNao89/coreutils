...
pub struct PathData {
    ...
    /// Track bytes read to detect new data availability
    #[allow(dead_code)]
    pub bytes_read: u64,
}
...
impl PathData {
    ...
    pub fn from_other_with_path(data: Self, path: &Path) -> Self {
        ...
        let mut new_data = Self::new(reader, path.metadata().ok(), data.display_name.as_str());
        // Preserve polling state across path updates
        new_data.fallback_to_polling = data.fallback_to_polling;
        new_data.last_polled = data.last_polled;
        new_data.bytes_read = data.bytes_read;
        new_data
    }
}
