...
//! Baby-step Giant-step (BSGS) optimization for ECM Phase 2
//!
...
/// Simple point multiplication for ECM
fn simple_point_mult(
    k: u64,
    ...
    while k_shift > 0 {
        if k_shift & 1 == 1 {
            ...
            if let Some(factor) = check_factor_gcd(&result.z, n) {
...
pub fn ecm_phase2_optimized_point_checks(
    point: &PointProjective,
    n: &BigUint,
    a: &BigUint,
    b1: u64,
    ...
    if range > 10_000 {
        ...
        for i in 1..=log_steps.min(10) {
            let pos = b1 + (range * (i as u64) / log_steps).min(range - 1);
            samples.push(pos);
        }
    }
    ...
    for _ in 0..(range / 1000).min(100) {
        let pos = b1 + rng.gen_range(0..range);
        samples.push(pos);
    }
    ...
    for &k in samples.iter() {
        // Compute k*P
        let (k_point, factor) = simple_point_mult(k, point.clone(), n, a);
        ...
        if !k_point.z.is_zero() {
            ...
            if check_count % BATCH_SIZE == 0 {
                if let Some(f) = check_factor_gcd(&gcd_accum, n) {
                    if f > BigUint::one() && &f < n {
                        return Some(f);
                    }
                }
